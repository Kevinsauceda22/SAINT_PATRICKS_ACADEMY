...
const LibroDiario = () => {
  ...
  const fetchCuentas = async () => {
    try {
      const response = await fetch('http://74.50.68.87/api/catalogoCuentas', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const deudorResponse = await fetch('http://74.50.68.87/api/Librodiario', {
        method: 'POST',
        headers: {
      ...
      const acreedorResponse = await fetch('http://74.50.68.87/api/Librodiario', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleDelete = async (Cod_libro_diario) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`http://74.50.68.87/api/Librodiario/${Cod_libro_diario}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
