...
const CatalogoContable = () => {
  ...
  const fetchCuentas = async () => {
    try {
      const response = await axios.get('http://74.50.68.87:4000/api/catalogoCuentas', {
        headers: {
    ...
    const handleDelete = async (cuenta) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            await axios.delete(`http://74.50.68.87:4000/api/catalogoCuentas/${cuenta.Cod_cuenta}`, {
              headers: {
                'Authorization': 'Bearer ' + localStorage.getItem('token')
              }
            });
      ...
      try {
        ...
        if (editingCuenta.Cod_cuenta) {
          response = await axios.put(
            `http://74.50.68.87:4000/api/catalogoCuentas/${editingCuenta.Cod_cuenta}`,
            cuentaData,
            { headers }
          );
        } else {
