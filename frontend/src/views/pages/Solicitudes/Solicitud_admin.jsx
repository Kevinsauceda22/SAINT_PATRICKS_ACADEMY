...
const Solicitud = () => {
  ...
  const fetchSolicitudes = async () => {
    setLoading(true);
    try {
        const response = await fetch('http://74.50.68.87/api/Solicitud_admin/solicitudess');
...
const getIconByEstado = (estado) => {
    ...
    img.onload = async () => {
      ...
      try {
        const response = await fetch(
          `http://74.50.68.87/api/Solicitud_admin/solicitudes-persona/${selectedCita.id}`
        );
        if (!response.ok) {
  ...
  const handleEventClick = async (info) => {
    ...
    if (citaSeleccionada.estado === 'Pendiente') {
      try {
        const response = await fetch(`http://74.50.68.87/api/Solicitud_admin/solicitudes-persona/${citaId}`);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          const response = await fetch(`http://74.50.68.87/api/Solicitud_admin/solicitudes/${citaId}`, {
            method: 'PUT',
...
const fetchSolicitudConPersona = async (Cod_solicitud) => {
  try {
    const response = await fetch(`http://74.50.68.87/api/Solicitud_admin/solicitudes-persona/${Cod_solicitud}`);
    if (!response.ok) {
  ...
  const handleAcceptCita = async () => {
    try {
      ...
      const response = await fetch(
        `http://74.50.68.87/api/Solicitud_admin/solicitudes/${selectedCita.id}`,
        {
          method: 'PUT',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        selectedCita
          ? `http://74.50.68.87/api/Solicitud_admin/solicitudes/${selectedCita.id}`
          : 'http://74.50.68.87/api/Solicitud_admin/solicitudes',
        {
    ...
    img.onload = async () => {
      ...
      const updatedSolicitudes = await Promise.all(
        solicitudes.map(async (cita) => {
          try {
            const response = await fetch(`http://74.50.68.87/api/Solicitud_admin/solicitudes-persona/${cita.id}`);
            if (!response.ok) {
              throw new Error('Error al obtener detalles de la solicitud.');
            }
            const detailedData = await response.json();
