...
const ReporteActividades = () => {
  ...
  const cargarDatos = async () => {
    try {
      ...
      const response = await axios.get('http://74.50.68.87:4000/api/bitacora/reporte', config);
  ...
  const cargarUsuarios = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://74.50.68.87:4000/api/usuarios', {
  ...
  const cargarObjetos = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://74.50.68.87:4000/api/bitacora/objetos', {
        headers: { Authorization: `Bearer ${token}` }
      });
