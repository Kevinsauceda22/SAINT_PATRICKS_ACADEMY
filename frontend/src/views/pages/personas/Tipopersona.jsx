...
const TipoPersona = () => {
  ...
  const obtenerTiposPersona = async () => {
    try {
      const response = await fetch('http://74.50.68.87/api/tipopersona/tipo-persona');
  ...
  const actualizarTipoPersona = async (codTipo, nuevoTipo) => {
    try {
      const response = await fetch(`http://74.50.68.87/api/tipopersona/tipo-persona/${codTipo}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
