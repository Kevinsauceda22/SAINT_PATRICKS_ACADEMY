...
const handleCreatePersona = async () => {
  ...
  const handleDeletePersona = async () => {
    try {
      const response = await fetch(`http://localhost:4000/api/personas/${encodeURIComponent(personaToDelete.cod_persona)}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
      });
      ...
      if (response.ok) {
        ...
        swal.fire({ icon: 'success', title: 'Eliminación exitosa', text: 'La persona ha sido eliminada correctamente.' });
      } else {
  ...
  const exportToPDF = () => {
    ...
    doc.autoTable({
      head: [['#', 'DNI', 'Nombre', 'Apellido', 'Nacionalidad', 'Dirección', 'Fecha de Nacimiento', 'Estado', 'Tipo de Persona', 'Departamento', 'Género']],
    ...
    <CTableBody>
      {currentRecords.map((persona, index) => (
        <CTableRow key={persona.cod_persona}>
          ...
          <CTableDataCell>{persona.Nombre.toUpperCase()}</CTableDataCell>
          <CTableDataCell>{persona.Segundo_nombre.toUpperCase()}</CTableDataCell>
          <CTableDataCell>{persona.Primer_apellido.toUpperCase()}</CTableDataCell>
          <CTableDataCell>{persona.Segundo_apellido.toUpperCase()}</CTableDataCell>
          <CTableDataCell>{persona.Nacionalidad.toUpperCase()}</CTableDataCell>
          <CTableDataCell>{persona.direccion_persona.toUpperCase()}</CTableDataCell>
          <CTableDataCell>{persona.fecha_nacimiento}</CTableDataCell>
          <CTableDataCell>{persona.Estado_Persona.toUpperCase()}</CTableDataCell>
          ...
          <CTableDataCell className="text-end">
                        ...
                        size="sm"
                        style={{ opacity: 0.8 }}  // Ajusta la opacidad
                        onClick={() => confirmDelete(persona.cod_persona)}
                        >
                        <CIcon icon={cilTrash} />
                        </CButton>
                    </CTableDataCell>
    ...
    <CForm>
      ...
      <CModalFooter>
        ...
        <CInputGroup className="mb-3">
            ...
            onChange={(e) => setPersonaToUpdate({ ...personaToUpdate, dni_persona: e.target.value })}
            required
        />
      </CInputGroup>
      <CInputGroup className="mb-3">
      ...
      <CInputGroup className="mb-3">
        ...
        <CFormInput
          ...
          onChange={(e) => setpersonaToUpdate({ ...personaToUpdate, Segundo_nombre: e.target.value })}
        />
      </CInputGroup>
      <CInputGroup className="mb-3">
        <CInputGroupText>Primer Apellido</CInputGroupText>
        <CFormInput
          ...
          onChange={(e) => setPersonaToUpdate({ ...personaToUpdate, Primer_apellido: e.target.value })}
          required
        />
      </CInputGroup>
      <CInputGroup className="mb-3">
        ...
        <CFormInput
          ...
          onChange={(e) => setPersonaToUpdate({ ...personaToUpdate, Segundo_apellido: e.target.value })}
        />
      </CInputGroup>
      <CInputGroup className="mb-3">
        <CInputGroupText>Nacionalidad</CInputGroupText>
        <CFormInput
          ...
          onChange={(e) => setpersonaToUpdate({ ...personaToUpdate, Nacionalidad: e.target.value })}
          required
        />
      </CInputGroup>
      <CInputGroup className="mb-3">
        ...
        <CFormInput
          ...
          onChange={(e) => setpersonaToUpdate({ ...personaToUpdate, direccion_persona: e.target.value })}
        />
      </CInputGroup>
      <CInputGroup className="mb-3">
        <CInputGroupText>Fecha de Nacimiento</CInputGroupText>
        <CFormInput
          ...
          onChange={(e) => setpersonaToUpdate({ ...personaToUpdate, fecha_nacimiento: e.target.value })}
          required
        />
      </CInputGroup>
      <CModalFooter>
        ...
        {/* Modal para Borrar Persona */}
                <CModal visible={personaToDelete} onClose={() => setModalDeleteVisible(false)} backdrop="static">
                <CModalHeader closeButton>
                    <CModalTitle>Eliminar Persona</CModalTitle>
                </CModalHeader>
                <CModalBody>
                    <p>¿Estás seguro de que deseas eliminar a la persona {personaToDelete.codigo_persona}? Esta acción no se puede deshacer.</p>
                </CModalBody>
                <CModalFooter>
                    <CButton 
                    style={{ backgroundColor: '#6c757d', color: 'white', borderColor: '#6c757d' }} 
