...
{/*******************************************************************************************************************/}
  ...
  const fetchContactos = async () => {
    try {
      const response = await fetch('http://74.50.68.87/api/contacto/obtenerContacto');
  ...
  const fetchTiposContacto = async () => {
    try {
      const response = await fetch('http://74.50.68.87/api/tipoContacto/obtenerTipoContacto');
      if (!response.ok) throw new Error(`Error en la solicitud: ${response.statusText}`);
      const data = await response.json();
...
{/*******************************************************FUNCION PARA CREAR Y ACTUALIZAR**************************************************/}
  const handleCreateOrUpdate = async () => {
    ...
    const url = contactoToUpdate
      ? `http://74.50.68.87/api/contacto/actualizarContacto/${contactoToUpdate.cod_contacto}`
      : 'http://74.50.68.87/api/contacto/crearContacto';
  ...
  const handleDeleteContacto = async (cod_contacto, descripcionContacto) => {
    try {
      ...
      const response = await fetch(
        `http://74.50.68.87/api/contacto/eliminarContacto/${encodeURIComponent(cod_contacto)}`,
        { method: 'DELETE' }
      );
