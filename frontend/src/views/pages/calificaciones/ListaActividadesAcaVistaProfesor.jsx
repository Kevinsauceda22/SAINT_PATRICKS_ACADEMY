...
//para paginacion y busqueda de la vista parciales
  ...
  const fetchCodProfesor = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://74.50.68.87/api/profesores/obtenerCodProfesor', {
  ...
  const fetchSecciones = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://74.50.68.87/api/seccion/porprofesores', {
        method: 'GET',
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await response.json();
  ...
  const fetchAsignaturas = async (codSeccion) => {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://74.50.68.87/api/asignaturas/porprofesor/${codSeccion}`, {
            headers: { Authorization: `Bearer ${token}` },
        });
...
const fetchListaCiclo = async () => {
  try {
    ...
    const response = await fetch('http://74.50.68.87/api/actividadesAcademicas/obtenerPonderacionesPorProfesor', {
      method: 'GET',
      headers: {
...
const fetchActividades = async () => {
  try {
      ...
      const response = await fetch(
          `http://74.50.68.87/api/actividadesAcademicas/actividadesporparcialseccion?codSeccion=${selectedSeccion.Cod_secciones}&codParcial=${selectedParcial.Cod_parcial}&codAsignatura=${selectedAsignatura.Cod_asignatura}`,
          {
              method: 'GET',
              headers: { Authorization: `Bearer ${token}` },
          }
...
const obtenerCodCicloPorPonderacionCiclo = async (Cod_ponderacion_ciclo) => {
  try {
    const response = await fetch(
      `http://74.50.68.87/api/ponderacionCiclo/ciclo?Cod_ponderacion_ciclo=${Cod_ponderacion_ciclo}`
    );
    const data = await response.json();
