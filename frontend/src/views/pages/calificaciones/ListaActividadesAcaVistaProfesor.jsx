...
//para paginacion y busqueda de la vista parciales
  ...
  const fetchSecciones = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:4000/api/asigsec/porprofesor', {
        method: 'GET',
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await response.json();
...
const handleGestionarActividades = (parcial) => {
  ...
  const handleCrearActividad = async () => {
    ...
    if (!esValido) {
      ...
      const actividadData = {
        ...nuevaActividad,
        Fechayhora_Inicio: formatDateForMySQL(nuevaActividad.Fechayhora_Inicio),
        Fechayhora_Fin: formatDateForMySQL(nuevaActividad.Fechayhora_Fin),
        Cod_secciones: selectedSeccion?.Cod_secciones,
    ...
    const updateResponse = await fetch(
      `http://localhost:4000/api/actividadesAcademicas/${Cod_actividad_academica}`,
      {
        ...
        body: JSON.stringify({
          Nombre_actividad_academica,
          Descripcion,
          Fechayhora_Inicio: formatDateForMySQL(actividadToUpdate.Fechayhora_Inicio),
          Fechayhora_Fin: formatDateForMySQL(actividadToUpdate.Fechayhora_Fin),
          Valor,
        }),
      }
    );
