...
const ListaInstitutos = () => {
  ...
  const generarReportePDF = () => {
    ...
    if (!filteredInstituto || filteredInstituto.length === 0) {
      swal.fire({
        icon: 'info',
        title: 'Tabla vacía',
    ...
    img.onload = () => {
      ...
      doc.autoTable({
        startY: yPosition + 4,
        head: [['#', 'Nombre']],
        body: filteredInstituto.map((instituto, index) => [
          instituto.originalIndex || index + 1,
  ...
  const generarReporteExcel = () => {
    ...
    const filas = filteredInstituto.map((instituto, index) => [
      instituto.originalIndex || index + 1,
