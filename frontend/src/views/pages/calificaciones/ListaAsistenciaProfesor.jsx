...
useEffect(() => {
  ...
  const fetchSecciones = async () => {
    try {
        ...
        const response = await fetch('http://74.50.68.87/api/seccionalumno/seccionesporprofe', {
            method: 'GET',
            headers: {
  ...
  const fetchEstadosAsistencia = async () => {
    try {
      const response = await fetch('http://74.50.68.87/api/estadoAsistencia/estadoasistencias');
  ...
  const fetchAlumnosPorSeccion = async (codSeccion) => {
    try {
      const response = await fetch(`http://74.50.68.87/api/seccionalumno/estudiantes/${codSeccion}`);
  ...
  const fetchTodasAsistencias = async (fecha) => {
    try {
      ...
      const response = await fetch(`http://74.50.68.87/api/asistencia/asistencias?cod_seccion=${codSeccionSeleccionada}&fecha=${fechaFormateada}`);
      if (!response.ok) {
  ...
  const fetchRecuentoAsistencias = async () => {
    ...
    try {
      const response = await fetch(`http://74.50.68.87/api/asistencia/recuento?codSeccion=${codSeccionSeleccionada}`);
  ...
  try {
    ...
    const response = await fetch(`http://74.50.68.87/api/asistencia/asistencias?cod_seccion=${codSeccionSeleccionada}&fecha=${formattedDate}`);
...
const handleObservacionChangeActualizar = (index, value) => {
  ...
  const handleGuardarAsistencias = async () => {
    try {
      ...
      const verificarResponse = await fetch('http://74.50.68.87/api/asistencia/verificarExistencia', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const response = await fetch('http://74.50.68.87/api/asistencia/crearasistencias', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const handleActualizarAsistencias = async () => {
    try {
      ...
      const response = await fetch('http://74.50.68.87/api/asistencia/actualizarasistencias', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
    ...
    const handleViewAsistencia = async (Cod_secciones, nombreSeccion,grado,anio) => {
      ...
      if (Cod_secciones) {
        try {
            ...
            const response = await fetch(`http://74.50.68.87/api/seccionalumno/nomenclatura?codSeccion=${Cod_secciones}`);
          ...
          <CRow className="align-items-center mb-5">
            <CCol xs="12" className="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
              <div className="flex-grow-1 text-center">
                <h4 className="text-center fw-semibold pb-2 mb-0" style={{display: "inline-block", borderBottom: "2px solid #4CAF50" }}>Asistencia: Mis Secciones</h4>
              </div>
                  ...
                  <CTableRow key={index}>
                    ...
                    <CTableDataCell>
                      <CButton
                        ...
                        onClick={() => handleViewAsistencia(seccion.Cod_secciones, seccion.Seccion,seccion.Grado,seccion.Anio_Academico)}
                      >
                        Ver Asistencia
                      </CButton>
                    </CTableDataCell>
...
export default ListaAsistenciaProfesor;
