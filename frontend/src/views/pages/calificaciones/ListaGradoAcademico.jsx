...
const ListaGradoAcademico = () => {
  ...
  const handleInputChange = (e, setFunction) => {
    ...
    const regex =/^[A-ZÑÁÉÍÓÚ\s,]*$/; // Solo letras, espacios y la Ñ
 ...
 // Cambiar página
  ...
  const handleSearch = (event) => {
    ...
    const regex = /^[A-ZÑÁÉÍÓÚ\s,]*$/; // Solo letras, acentos, ñ, espacios y comas
...
//--------------------------------------------- REPORTE PDF Y EXCEL -----------------------
  const generarReporteGradosAcademicosPDF = () => {
    ...
    if (!filteredGradosAcademicos || filteredGradosAcademicos.length === 0) {
      swal.fire({
        icon: 'info',
        title: 'Tabla vacía',
    ...
    img.onload = () => {
      ...
      doc.autoTable({
        ...
        body: filteredGradosAcademicos.map((gradoAcademico, index) => [
          gradoAcademico.originalIndex || index + 1, // Usar el índice original en lugar del índice basado en la paginación
  ...
  const generarReporteExcel = () => {
    ...
    const filas = filteredGradosAcademicos.map((grado, index) => [
      grado.originalIndex || index + 1, // Índice basado en la posición de los datos filtrados
