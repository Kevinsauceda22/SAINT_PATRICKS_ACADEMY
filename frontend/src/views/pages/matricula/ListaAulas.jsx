...
const ListaAulas = () => {
  ...
  useEffect(() => {
    ...
    const fetchEdificios = async () => {
      try {
          const response = await fetch('http://74.50.68.87:4000/api/aula/edificio');
  ...
  const fetchAulas = async () => {
    try {
      const response = await fetch('http://74.50.68.87:4000/api/aula/aulas');
      const data = await response.json();
      setAulas(data);
    } catch (error) {
  ...
  if (data.p_Secciones_ocupadas > data.p_Secciones_disponibles) {
    ...
    try {
      const response = await fetch('http://74.50.68.87:4000/api/aula/crear_aula', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateAula = async () => {
    ...
    try {
      const response = await fetch(`http://74.50.68.87:4000/api/aula/actualizar_aula/${aulaToUpdate.Cod_aula}`, {
        method: 'PUT',
  ...
  const handleDeleteAula = async () => {
    try {
      const response = await fetch(`http://74.50.68.87:4000/api/aula/${aulaToDelete.Cod_aula}`, {
        method: 'DELETE',
      });
