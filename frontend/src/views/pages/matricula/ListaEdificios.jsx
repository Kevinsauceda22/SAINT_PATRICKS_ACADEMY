...
const ListaEdificios = () => {
  ...
  const fetchEdificios = async () => {
    try {
      const response = await fetch('http://74.50.68.87i/edificio/edificios'); // Realiza la petición al backend
  ...
  const handleCreateEdificio = async () => {
    ...
    try {
      const response = await fetch('http://74.50.68.87i/edificio/crear_edificio', {
        method: 'POST',
  ...
  const handleUpdateEdificio = async () => {
    ...
    try {
      const response = await fetch('http://74.50.68.87i/edificio/actualizar_edificio', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleDeleteEdificio = async () => {
    try {
      const response = await fetch(`http://74.50.68.87i/edificio/${encodeURIComponent(edificioToDelete.Cod_edificio)}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
      });
