...
const EstadoMatricula = () => {
  ...
  const obtenerEstados = async () => {
    try {
      const response = await fetch('http://74.50.68.87/api/estadomatricula/estado-matricula');
  ...
  const eliminarEstado = async (codEstado) => {
    try {
      const response = await fetch(`http://74.50.68.87/api/estadomatricula/estado-matricula/${codEstado}`, {
        method: 'DELETE',
      });
