...
const ListaGestion_Academica = () => {
  ...
  const fetchAgrupadores = async () => {
    try {
      const response = await fetch('http://74.50.68.87/api/gestion_academica/obtenerTodasAgrupaciones');
  ...
  const handleDescargarPDF = async (Cod_periodo_matricula) => {
    try {
      ...
      const responsePeriodo = await fetch(`http://74.50.68.87/api/gestion_academica/detalle/${Cod_periodo_matricula}`);
      ...
      const responseSecciones = await fetch(`http://74.50.68.87/api/gestion_academica/secciones_por_periodo/${Cod_periodo_matricula}`);
  ...
  const handleGuardarAgrupador = async () => {
    try {
      const response = await fetch('http://74.50.68.87/api/gestion_academica/crear_agrupador', {
        method: 'POST',
      });
