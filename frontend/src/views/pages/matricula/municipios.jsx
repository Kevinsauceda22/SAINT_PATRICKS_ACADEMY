...
const MunicipioMantenimiento = () => {
  ...
  const obtenerMunicipios = async () => {
    try {
      const response = await fetch('http://74.50.68.87/api/departamento/municipios');
  ...
  const obtenerDepartamentos = async () => {
    try {
      const response = await fetch('http://74.50.68.87/api/departamento/departamentos');
      const data = await response.json();
      if (response.ok) {
...
const handleInputChange = (e) => {
  ...
  const actualizarMunicipio = async () => {
    ...
    try {
      const response = await fetch(`http://74.50.68.87/api/departamento/municipios/${codMunicipio}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
