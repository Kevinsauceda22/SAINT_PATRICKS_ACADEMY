...
const TipoMatricula = () => {
  ...
  const obtenerTipos = async () => {
    try {
      const response = await fetch('http://74.50.68.87/api/tipomatricula/tipo-matricula');
  ...
  const actualizarTipo = async (codTipo, nuevoTipo) => {
    try {
      const response = await fetch(`http://74.50.68.87/api/tipomatricula/tipo-matricula/${codTipo}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
