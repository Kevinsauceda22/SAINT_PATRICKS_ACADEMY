...
const ListaActividades = () => {
  ...
  useEffect(() => {
   ...
   const fetchActividades = async (Cod_actividades_extracurriculares = 0 ) => {
    try {
      const response = await fetch(`http://74.50.68.87:4000/api/actividades/extracurriculares/${Cod_actividades_extracurriculares}`);
  ...
  const fetchSecciones = async () => {
    try {
      const response = await fetch('http://74.50.68.87:4000/api/secciones/periodo-activo');
      const data = await response.json();
  ...
  if (nuevoEstado === 'Cancelada') {
    swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await fetch('http://74.50.68.87:4000/api/actividades/cambiar_estado', {
            method: 'PUT',
...
const handleCreateActividad = async () => {
  ...
  try {
    const response = await fetch('http://74.50.68.87:4000/api/actividades/extracurriculares', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        p_Nombre: Nombre,
