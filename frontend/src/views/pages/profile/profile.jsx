...
const UserProfile = () => {
  ...
  const MAX_ATTEMPTS = 3;
   ...
   const API_BASE_URL = 'http://74.50.68.87:4000/api';
  ...
  const generateQR2FA = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://74.50.68.87:4000/api/usuarios/enableTwoFactorAuth/${auth.cod_usuario}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleVerify = async () => {
    try {
      ...
      const response = await fetch('http://74.50.68.87:4000/api/usuarios/verifyTwoFactorAuthCode', {
        method: 'POST',
        headers: {
  ...
  const disableTwoFactor = async () => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await axios.post(
          'http://74.50.68.87:4000/api/usuarios/disableTwoFactorAuth',
          { cod_usuario: auth.cod_usuario },
          { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
        );
