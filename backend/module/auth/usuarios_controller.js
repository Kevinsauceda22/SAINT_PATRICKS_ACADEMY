...
// Controller function to change a user's role
export const cambiarRolUsuario = async (req, res = response) => {
    ...
    if (!userId || !newRoleId) {
        return res.status(400).json({
            ok: false,
            mensaje: 'El ID de usuario y el nuevo rol son requeridos'
        });
    }
    ...
    try {
        ...
        const [roleExists] = await pool.query(
            `SELECT Cod_rol FROM tbl_roles WHERE Cod_rol = ? AND Ind_Rol = '1'`,
            [newRoleId]
        );
        ...
        const [result] = await pool.query(
            `UPDATE tbl_usuarios SET Cod_rol = ? WHERE cod_usuario = ?`,
            [newRoleId, userId]
        );
...
export const listarRoles = async (req, res = response) => {
    try {
        // Consultar solo los roles activos
        const [roles] = await pool.query(
            `SELECT Cod_rol, Nom_rol, Descripcion 
             FROM tbl_roles 
             WHERE Ind_Rol = '1'
             ORDER BY Nom_rol`
        );
